...
window.connectUSBDevice = async function (e) {
    ...
    if (!("serial" in navigator)) {
        alert("⚠️ Pelayar web anda tidak menyokong Web Serial API.\nSila gunakan pelayar Google Chrome, MS Edge, atau Opera melalui HTTPS atau http://localhost.");
        return;
    }

    try {
        if (usbStatusText) usbStatusText.innerText = "Memilih Port USB...";
        ...
        window.logUSBTerminal("✅ Berjaya disambungkan ke USB Serial!");
        ...
        if (errName === 'NetworkError' || errMsg.includes('Failed to open') || errMsg.includes('busy') || errMsg.includes('resource')) {
            alert("⚠️ Gagal membuka Port USB!\n\nSebab biasa: Serial Monitor di perisian Arduino IDE masih TERBUKA.\n\n👉 Sila TUTUP Serial Monitor / Serial Plotter di Arduino IDE terlebih dahulu, kemudian cuba tekan butang USB Serial ini semula.");
            return;
        }
