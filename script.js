    ...
    try {
        ...
        try {
            ...
            try {
                ...
                // Pilihan 3: Jalankan secara terus jika di Android Chrome native navigator.serial gagal
                if (navigator.serial && isMobileOrTablet) {
                    console.log("📱 Native navigator.serial gagal di Android Chrome. Cuba Polyfill semula...");
                    if (window.serial && window.serial.Serial) {
                        const fallbackPoly = new window.serial.Serial();
                        usbPort = await fallbackPoly.requestPort({ filters: [] });
                    } else {
                        throw err2;
                    }
                } else {
