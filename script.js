...
window.connectUSBDevice = async function (e) {
    ...
    try {
        ...
        try {
            // Pilihan 1: Minta port tanpa penapis terlebih dahulu
            usbPort = await navigator.serial.requestPort();
        } catch (filterErr) {
            // Pilihan 2: Jika gagal atau Android Chrome perlukan penapis khas, cuba dengan filters
            if (filterErr && (filterErr.name === 'NotFoundError' || String(filterErr).includes('No device'))) {
                usbPort = await navigator.serial.requestPort({ filters: commonUsbFilters });
            } else {
                throw filterErr;
            }
        }
