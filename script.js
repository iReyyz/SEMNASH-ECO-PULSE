...
window.connectUSBDevice = async function (e) {
    ...
    // Fungsi pembantu untuk mendapatkan enjin Web Serial (Native atau WebUSB Polyfill untuk Android)
    function getSerialEngine() {
        // 1. Jika di Android/Mobile dan pustaka web-serial-polyfill dimuatkan (window.serial.Serial)
        ...
        if (typeof window.Serial === 'function') {
            console.log("📱 Menggunakan enjin window.Serial (WebUSB Polyfill)...");
            return new window.Serial();
        }
        if (typeof window.SerialPolyfill === 'function') {
            return new window.SerialPolyfill();
        }
        // 2. Default ke native navigator.serial (Desktop Chrome / MS Edge)
    ...
    try {
        ...
        // Senarai Vendor ID Cip USB Arduino & ESP32 (Sertakan kedua-dua vendorId untuk WebUSB dan usbVendorId untuk WebSerial)
        const commonUsbFilters = [
            ...
            {} // Penapis kosong untuk membenarkan sebarang peranti USB terhubung
        ];

        try {
            // Pilihan 1: Minta port menerusi serialInstance (Polyfill / Native)
            usbPort = await serialInstance.requestPort({ filters: commonUsbFilters });
        } catch (filterErr) {
            try {
                // Pilihan 2: Cuba tanpa filters jika requestPort ber-filter ditolak
                usbPort = await serialInstance.requestPort();
            } catch (err2) {
                // Pilihan 3: Jalankan secara terus navigator.usb.requestDevice ({ filters: [] }) pada Android Chrome
                if (navigator.usb) {
                    console.log("📱 Menjalankan WebUSB navigator.usb.requestDevice secara terus...");
                    try {
                        ...
                        if (device) {
                            ...
                            isUSBConnected = true;
                            updateUSBUI(true, device.productName || "Arduino OTG (WebUSB)");
                            window.logUSBTerminal(`✅ Terhubung via WebUSB: ${device.productName || 'USB Device'}`);

                            // Mock Serial Writer untuk WebUSB Direct
                            usbPort = {
                                writable: {
                                    getWriter: () => ({
                                        write: async (data) => {
                                            try {
                                                ...
                                                const epNum = endpoint ? endpoint.endpointNumber : 1;
                                                await device.transferOut(epNum, data);
                                            } catch (e) { console.warn("WebUSB write warning:", e); }
                                        },
                                        releaseLock: () => { }
                                    })
                                }
                        ...
                        console.warn("Direct WebUSB failed:", usbErr);
                        throw err2;
                    }
                } else {
                    throw err2;
        ...
        // Hantar isyarat ujian automatik selepas 1.5 saat (selepas Arduino selesaikan reset bootloader)
        setTimeout(async () => {
            window.logUSBTerminal("🧪 Mengirim isyarat ujian 'O' ke Arduino...");
            await window.sendUSBSignal("O\n");
        }, 1500);

        // Listener jika USB kabel terputus secara tidak sengaja
        usbPort.addEventListener('disconnect', () => {
            isUSBConnected = false;
            isReadingUSB = false;
            usbPort = null;
            updateUSBUI(false);
            window.logUSBTerminal("⚠️ Sambungan kabel USB telah terputus.");
        });
...
// ⌨️ Tangkap Kekunci 'Enter' secara automatik pada dokumen untuk hantar isyarat USB
document.addEventListener('keydown', async function (e) {
    // Jangan ganggu jika pengguna sedang menaip dalam kotak input teks atau textarea
    ...
    if (e.key === 'Enter' && !isTypingInField) {
        console.log("⌨️ Kekunci ENTER ditekan pada paparan. Menghantar isyarat Buka Tong 'O' via USB Serial...");
        await window.sendUSBSignal("O\n");
    }
});
