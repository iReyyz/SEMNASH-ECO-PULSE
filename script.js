// GLOBAL BLUETOOTH & USB SERIAL DECLARATIONS
...
// GLOBAL USB SERIAL (WEB SERIAL API) VARIABLES
var usbPort = null;
var usbWriter = null;
var isUSBConnected = false;

window.logUSBTerminal = function (msg) {
    console.log(msg);
    const terminalLogs = document.getElementById('usbAdminTerminalLogs') || document.getElementById('usbTerminalLogs');
    if (terminalLogs) {
...
let isReadingUSB = false;
async function startUSBReader() {
    if (!usbPort || !usbPort.readable) return;
    isReadingUSB = true;
    ...
    try {
        ...
        window.logUSBTerminal("🟢 Mendengar maklum balas daripada Arduino USB Serial...");
        ...
        while (isReadingUSB) {
            ...
            if (value) {
                window.logUSBTerminal(`[ARDUINO]: ${value.trim()}`);
            }
        }
    } catch (err) {
        console.warn("Ralat Pembaca USB Serial:", err);
    }
}

/**
 * 🔌 Sambung ke Peranti USB Serial (Arduino UNO / ESP32)
 */
window.connectUSBDevice = async function (e) {
    if (e && e.preventDefault) e.preventDefault();

    const usbStatusText = document.getElementById('usbStatusText');
    const usbStatusDot = document.getElementById('usbStatusDot');

    function updateUSBUI(connected, portName = '') {
        if (usbStatusDot) {
            usbStatusDot.style.background = connected ? "#10b981" : "#ef4444";
        }
        if (usbStatusText) {
            usbStatusText.innerText = connected
                ? `🔌 USB: ${portName || 'Terhubung'}`
                : "🔌 USB Serial (Terputus)";
        }
    }
    ...
    // Tutup port terdahulu jika pengguna menekan butang sambung semula
    if (usbPort) {
        try {
            isReadingUSB = false;
            await usbPort.close();
        } catch (closeErr) {
            console.log("Menutup port USB lama...", closeErr);
        }
        usbPort = null;
        isUSBConnected = false;
    }

    // Pilih implementasi Web Serial: utamakan native navigator.serial pada Desktop, atau WebUSB Polyfill pada Android OTG
    function getSerialEngine() {
        if (!isMobileOrTablet && navigator.serial) {
            console.log("💻 Menggunakan enjin Native navigator.serial (Desktop Web Serial API)...");
            return navigator.serial;
        }
        if (window.serial && typeof window.serial.Serial === 'function') {
            console.log("📱 Menggunakan enjin window.serial.Serial (WebUSB Polyfill) untuk Android OTG...");
            return new window.serial.Serial();
        }
        if (typeof window.Serial === 'function') {
            return new window.Serial();
        }
        if (typeof window.SerialPolyfill === 'function') {
            return new window.SerialPolyfill();
        }
        return navigator.serial;
    }

    let serialInstance = getSerialEngine();

    if (!serialInstance && !("usb" in navigator)) {
        if (isIOS) {
            alert("⚠️ Peranti iPad / iOS (Safari) tidak menyokong USB Web Serial API.\n\n👉 Sila gunakan sambungan Web Bluetooth pada peranti iOS/iPad.");
        } else {
            alert("⚠️ Pelayar web anda tidak menyokong Web Serial API atau WebUSB.\n\n👉 Sila gunakan pelayar Google Chrome atau MS Edge (melalui HTTPS) pada Android tablet atau PC.");
        }
        return;
    }

    try {
        if (usbStatusText) usbStatusText.innerText = "Memilih Port USB (OTG)...";

        // Senarai Vendor ID Cip USB Arduino & ESP32
        const commonUsbFilters = [
            { usbVendorId: 0x2341, vendorId: 0x2341 }, // Arduino Official (Uno, Mega, Nano Every, etc.)
            { usbVendorId: 0x1a86, vendorId: 0x1a86 }, // CH340 / CH341 (Arduino Clone / Nano)
            { usbVendorId: 0x10c4, vendorId: 0x10c4 }, // CP2102 / CP2104 (ESP32 / NodeMCU)
            { usbVendorId: 0x0403, vendorId: 0x0403 }, // FTDI FT232 (Arduino Duemilanove / FTDI adapter)
            { usbVendorId: 0x2e8a, vendorId: 0x2e8a }, // Raspberry Pi Pico
            { usbVendorId: 0x303a, vendorId: 0x303a }  // Espressif System Native USB (ESP32-S2 / S3)
        ];

        try {
            // Pilihan 1: Minta port tanpa penapis dahulu
            usbPort = await serialInstance.requestPort();
        } catch (filterErr) {
            try {
                // Pilihan 2: Cuba dengan penapis vendorId & usbVendorId jika tanpa penapis gagal
                usbPort = await serialInstance.requestPort({ filters: commonUsbFilters });
            } catch (err2) {
                // Pilihan 3: Jalankan secara terus WebUSB jika polyfill dimuatkan
                if (window.serial && window.serial.Serial) {
                    window.logUSBTerminal("📱 Cuba WebUSB polyfill direct prompt...");
                    const poly = new window.serial.Serial();
                    usbPort = await poly.requestPort();
                } else {
                    throw err2;
                }
            }
        ...
        await usbPort.open({ baudRate: 9600 }); // Baud rate 9600 standard Arduino UNO / ESP32 Serial
        isUSBConnected = true;

        updateUSBUI(true, "Arduino / ESP32");
        window.logUSBTerminal("✅ Berjaya disambungkan ke USB Serial (OTG)!");

        // Mula membaca data balasan dari Arduino secara real-time
        startUSBReader();

        // Ujian USB sedia jika dipanggil secara manual oleh Mod Guru
        window.logUSBTerminal("ℹ️ Kabel USB Serial bersedia.");
    } catch (err) {
        console.error("Ralat Sambungan USB Serial:", err);
        updateUSBUI(false);

        const errMsg = String(err && err.message ? err.message : err);
        const errName = err && err.name ? err.name : '';

        // 1. Apabila peranti USB tidak dijumpai atau pengguna membatalkan pilihan
        if (errName === 'NotFoundError' || errMsg.includes('No compatible device') || errMsg.includes('No device')) {
            console.warn("Port USB tidak dijumpai atau tiada peranti serasi:", err);

            const choice = confirm(
                "⚠️ Peranti USB tidak dijumpai oleh Chrome Android.\n\n" +
                "💡 SEBAB UTAMA DI ANDROID:\n" +
                "1. Aplikasi 'Serial USB Terminal' mungkin telah ditetapkan sebagai 'Default App' oleh Android OS.\n" +
                "   👉 Sila buka Settings Tablet -> Apps -> Serial USB Terminal -> 'Clear Defaults', kemudian cabut & cacak semula OTG.\n\n" +
                "2. Chrome Android menyekat USB jika laman dibuka terus dari fail (file://).\n\n" +
                "------------------------------------------\n" +
                "Adakah anda mahu mengaktifkan MOD SIMULASI USB sekarang untuk menguji sistem?"
            );

            if (choice) {
                isUSBConnected = true;
                updateUSBUI(true, "Simulasi USB");
                alert("✅ Mod Simulasi USB Aktif! Sistem sedia menerima isyarat imbasan kod & kekunci ENTER.");
            }
            return;
        }

        if (errName === 'AbortError' || errMsg.includes('canceled')) {
            console.log("Pengguna membatalkan pilihan port USB.");
            return;
        }

        // 2. Port USB sedang digunakan oleh perisian lain
        if (errName === 'NetworkError' || errMsg.includes('Failed to open') || errMsg.includes('busy') || errMsg.includes('resource')) {
            alert("⚠️ Gagal membuka Port USB!\n\nSebab biasa: Port USB sedang dipegang oleh aplikasi lain (seperti Serial USB Terminal) atau Arduino IDE.\n\n👉 Sila TUTUP sepenuhnya aplikasi tersebut atau cabut & cacak semula OTG.");
            return;
        }

        // 3. Masalah kebenaran pelayar / protocol file://
        if (errName === 'SecurityError' || errMsg.includes('Security') || errMsg.includes('blocked')) {
            const enableSim = confirm("⚠️ Polisi Keselamatan Pelayar: Web Serial API disekat apabila laman dibuka terus dari fail (file://).\n\nAdakah anda mahu mengaktifkan MOD SIMULASI USB untuk menguji sistem sekarang?");
            if (enableSim) {
                isUSBConnected = true;
                updateUSBUI(true, "Simulasi USB");
                alert("✅ Mod Simulasi USB telah diaktifkan! Menekan kekunci ENTER akan memproses isyarat Buka Tong secara automatik.");
            }
            return;
        }

        if (isMobileOrTablet) {
            alert(`❌ Gagal Sambung USB (OTG Tablet):\n${errMsg}\n\n💡 Panduan Semakan USB OTG Tablet Android:\n1. Pastikan anda menggunakan Adapter USB OTG (Type-C ke USB Female).\n2. Buka Android Settings -> Cari "OTG" -> Pautkan / Aktifkan 'USB OTG' (terutamanya peranti Vivo/Oppo/Realme).\n3. Berikan kebenaran USB pop-up apabila pelayar Chrome memintanya.`);
        } else {
            alert(`❌ Ralat Sambungan USB:\n${errMsg}`);
        }
    }
};

/**
 * 🚀 Hantar Isyarat Buka / Tutup Tong Sampah melalui USB atau Bluetooth
 */
window.sendUSBSignal = async function (command = "O\n") {
    let sentSuccess = false;

    // 1. Hantar melalui Kabel USB Serial (Web Serial API) jika terhubung
    if (usbPort && usbPort.writable) {
        try {
            ...
            writer.releaseLock();
            window.logUSBTerminal(`[WEB SENT USB]: ${JSON.stringify(command)}`);
            sentSuccess = true;
        } catch (err) {
            console.error("❌ Gagal menghantar isyarat via USB Serial:", err);
            window.logUSBTerminal(`[USB ERROR]: ${err.message}`);
        }
    }

    // 2. Hantar melalui Bluetooth jika terhubung & USB tidak aktif
    if (!sentSuccess && bluetoothCharacteristic) {
        try {
            const encoder = new TextEncoder();
            await bluetoothCharacteristic.writeValue(encoder.encode(command));
            window.logUSBTerminal(`[WEB SENT BT]: ${JSON.stringify(command)}`);
            sentSuccess = true;
        } catch (err) {
            console.warn("⚠️ Ralat menghantar isyarat Bluetooth:", err);
            window.logUSBTerminal(`[BT ERROR]: ${err.message}`);
        }
    }

    // 3. Mod Simulasi fallback jika tiada peranti fizikal
    if (!sentSuccess) {
        window.logUSBTerminal(`[SIMULATOR RECV]: ${JSON.stringify(command)}`);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function extractUsedFromItem(b) {
        if (!b) return false;
        if (Array.isArray(b)) return Boolean(b[3]);
        if (typeof b !== 'object') return false;
        return Boolean(b.used || b.Used || b.isUsed || b.IsUsed || b.status === 'used' || b.status === 'Telah Guna' || b.status === 'USED');
    }

    window.extractCodeFromItem = extractCodeFromItem;
    window.extractUsedFromItem = extractUsedFromItem;
    ...
    function processBarcode(code) {
        ...
        if (!found) {
            const lastScanText = document.getElementById('lastScanCodeText');
            if (lastScanText) {
                lastScanText.textContent = `❌ SAMPAH TIDAK SAH ("${inputCode}")`;
        ...
        const lastScanText = document.getElementById('lastScanCodeText');
        if (lastScanText) {
            lastScanText.textContent = `✅ ${itemCode} (${itemCategory} • +${itemPoints} Mata)`;
            lastScanText.style.color = 'var(--neon-green)';
        }
    ...
    function submitManualBarcode() {
        if (typeof window.submitManualBarcode === 'function' && window.submitManualBarcode !== submitManualBarcode) {
        ...
        if (!val) return;
        input.value = '';
        if (typeof window.playScanBeep === 'function') {
            window.playScanBeep();
        }
        processBarcode(val);
    }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.keyCode === 13) {
            ...
            if (screen5 && (screen5.classList.contains('active') || screen5.style.display === 'flex' || screen5.style.display === 'block')) {
                // Abaikan jika masih dalam tempoh kunci 1000ms selepas imbasan (elak ekor Enter dari Scanner Gun)
                if (Date.now() < (window.screen5LockUntil || 0)) {
                    return;
                }
                const btnOpen = document.getElementById('btnSendBluetoothOpen');
                if (btnOpen) {
                    e.preventDefault();
                    if (typeof window.handleOpenBinAction === 'function') {
                        window.handleOpenBinAction();
                    } else {
                        btnOpen.click();
                    }
                }
    ...
    window.handleOpenBinAction = async function (e) {
        ...
        if (window.isReadyToOpenBin === false) {
            console.log("Tong sampah dikunci. Sila tekan Enter di Skrin 5 untuk membuka tong sampah.");
            return;
        }
        window.isReadyToOpenBin = false;
        ...
        // Hantar Isyarat Buka Tong ('O\n') ke USB Serial / Bluetooth Arduino
        if (typeof window.sendUSBSignal === 'function') {
            await window.sendUSBSignal("O\n");
        } else if (bluetoothGattServer && bluetoothGattServer.connected && bluetoothCharacteristic) {
            try {
                const encoder = new TextEncoder();
                await bluetoothCharacteristic.writeValue(encoder.encode('O\n'));
                console.log("Isyarat O dihantar ke Arduino via Bluetooth!");
            } catch (e) {
        ...
        const itemCodeAlpha = itemCodeUpper.replace(/[^A-Z0-9]/g, '');
        const itemCodeNum = itemCodeUpper.replace(/[^0-9]/g, '');

        if (Array.isArray(barcodes)) {
            barcodes.forEach(b => {
                const bCode = extractCodeFromItem(b);
                const bClean = String(bCode).toUpperCase();
                const bAlpha = bClean.replace(/[^A-Z0-9]/g, '');
                const bNum = bClean.replace(/[^0-9]/g, '');

                const isMatch = (bClean === itemCodeUpper) ||
                    (bAlpha && itemCodeAlpha && bAlpha === itemCodeAlpha) ||
                    (bNum && itemCodeNum && bNum.length >= 2 && bNum === itemCodeNum);
                ...
                if (isMatch) {
                    if (typeof b === 'object' && !Array.isArray(b)) {
                        ...
                        b.isUsed = true;
                        b.status = 'used';
                        b.usedBy = currentStudent ? currentStudent.name : 'Pelajar';
                        b.usedAt = new Date().toLocaleString('ms-MY');
