...
window.connectUSBDevice = async function (e) {
    ...
    // Pilih implementasi Web Serial: gunakan native navigator.serial atau SerialPolyfill (via WebUSB) untuk Android
    let serialInstance = navigator.serial;
    if (typeof window.SerialPolyfill === 'function' && (isMobileOrTablet || !serialInstance)) {
        console.log("📱 Menggunakan WebSerial Polyfill (WebUSB Engine) untuk Tablet Android...");
        serialInstance = new window.SerialPolyfill();
    }
    ...
    try {
        ...
        // Senarai Vendor ID Cip USB Arduino & ESP32 (Arduino Original, CH340, CP2102, FTDI, ESP32)
        const commonUsbFilters = [
            { usbVendorId: 0x2341 }, // Arduino Official (Uno, Mega, Nano Every, etc.)
            { usbVendorId: 0x1a86 }, // CH340 / CH341 (Arduino Clone / Nano)
            { usbVendorId: 0x10c4 }, // CP2102 / CP2104 (ESP32 / NodeMCU)
            { usbVendorId: 0x0403 }, // FTDI FT232 (Arduino Duemilanove / FTDI adapter)
            { usbVendorId: 0x2e8a }, // Raspberry Pi Pico
            { usbVendorId: 0x303a }  // Espressif System Native USB (ESP32-S2 / S3)
        ];

        try {
            // Pilihan 1: Minta port menerusi serialInstance
            ...
            try {
                ...
                // Pilihan 3: Jika native navigator.serial gagal pada Android, cuba paksa SerialPolyfill
                if (typeof window.SerialPolyfill === 'function' && serialInstance !== window._polySerial) {
                    window._polySerial = new window.SerialPolyfill();
                    usbPort = await window._polySerial.requestPort({ filters: commonUsbFilters });
                } else {
                    throw err2;
                }
            }
        ...
        // 1. Pengguna tekan Batal / Tutup tetingkap pilih port USB
        if (errName === 'NotFoundError' || errName === 'AbortError' || errMsg.includes('No port selected') || errMsg.includes('canceled')) {
            console.log("Pengguna membatalkan pilihan port USB.");
            return;
        }

        // 2. Port USB sedang digunakan oleh Arduino IDE (Serial Monitor terbuka)
        if (errName === 'NetworkError' || errMsg.includes('Failed to open') || errMsg.includes('busy') || errMsg.includes('resource')) {
            alert("⚠️ Gagal membuka Port USB!\n\nSebab biasa: Serial Monitor di perisian Arduino IDE masih TERBUKA atau port digunakan aplikasi lain.\n\n👉 Sila TUTUP Serial Monitor di Arduino IDE / aplikasi lain terlebih dahulu.");
            return;
        }
